...
def scene():
    return jsonify({
        "objects": {
            "red":   {"position":[0.4,0.0,0.14],"reachability":{"severity":"ok","distance":0.4,"message":"Reachable"}},
            "blue":  {"position":[0.3,0.25,0.14],"reachability":{"severity":"ok","distance":0.39,"message":"Reachable"}},
            "green": {"position":[0.3,-0.25,0.14],"reachability":{"severity":"ok","distance":0.39,"message":"Reachable"}}
        },
        "robot":{"position":[0.0,0.0,0.6],"holding":None},
        "obstacles":[],"estop":False,"agent_running":False,
        "log":[{"type":"info","text":"Demo mode — robot not connected"}]
    })
...
def reachability():
    ...
    y = float(request.args.get("y", 0))
    d = math.sqrt(x**2 + y**2)
    s = "error" if d > 0.75 else "warning" if d > 0.65 or d < 0.25 else "ok"
    return jsonify({"severity": s, "distance": round(d,3), "message": f"{d:.2f}m from base"})
